...
double basic_growthrate; 	// intrinsic plant growth rate; is actually "quality * growthrate" due to the way it's implemented in the growth function
int version; 			// 1 = ratio dependent, 0 = "standard" version of the model
double mutationstep_plants_1;	// speed of evolution (mutation step size) plant 1
double mutationstep_plants_2;	// speed of evolution (mutation step size) plant 2
double costliness;		// costliness of defense
double effect_amount;		// effectiveness of defense in reducing consumption
double effect_toxicity;		// effectiveness of defense in toxicity
...
struct animal	//member variables of herbivore
{
	...
	double totaltoxicity;	//amount of toxic compounds consumed
};

struct plant	//member variables of plant
{
	double biomass;
	double defense;
