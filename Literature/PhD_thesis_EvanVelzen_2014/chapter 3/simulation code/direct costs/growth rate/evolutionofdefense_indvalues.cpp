...
#include <stdlib.h>
#include "random.h"
#include "evolutionofdefense_parameters.h"
#include "evolutionofdefense_indvalues.h"
#pragma hdrstop
...
void getparameters(ifstream &infile)
{
	...
	while (!infile.eof())
	{
		...
		if (line == 10)
			cost_growth_amount = strtod(stuff.c_str(), &pEnd);
		if (line == 11)
			cost_growth_toxicity = strtod(stuff.c_str(), &pEnd);
		if (line == 12)
		...
		for (int j = 0; j < numgenerations; j++)
 		{
			...
			{
				plantgrowth(plants);		  //plant growth
				consumption(plants, plant_order); //consumption of plants by herbivores
