...
void updateplant(plant &p)
{
	...
	p.competitiveness = 1.0 / (1.0 - (p.investment_amount * cost_ecological_amount + p.investment_toxicity * cost_ecological_toxicity));
	p.growthrate = basic_growthrate * (1.0 - (p.investment_amount * cost_direct_amount + p.investment_toxicity * cost_direct_toxicity));
}
//---------------------------------------------------------------------------
void initialize(vector<plant> &plants)
{
	...
	for (int i = 0; i < numlineages; i++)
	{
		...
		updateplant(plants[i]); //calculate competitiveness based on trait values
	}
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
double calculatecompetition(vector<plant> &p)
{
	//return total competition in the population
	double x = 0;
	for (int i = 0; i < numlineages; i++)
		x += p[i].biomass * p[i].competitiveness;
	return x;
}
//---------------------------------------------------------------------------
double averagecompetition(vector<plant> &p, double totalcompetition, double totalbiomass)
{
	//calculate and return competition each lineage faces, averaged over the entire population. Used to normalize competitiveness.
	double x = 0;
	double y = 0;
	for (int i = 0; i < numlineages; i++)
	{
       		y = (totalcompetition - p[i].biomass * p[i].competitiveness + p[i].biomass) / totalbiomass; //assuming intra-lineage competitiveness is always 1
		x += y * p[i].biomass / totalbiomass;
	}
	return x;
}
//---------------------------------------------------------------------------
void plantgrowth(vector<plant> &p)
{
	...
	double c1 = calculatecompetition(p);	//weighted sum of total competition in the population, if there is no normalization
	double totalbiomass = sumbiomass(p);	//total biomass in population
	double averagec = averagecompetition(p, c1, totalbiomass);	//weighted average of competition lineages face
	
	if (competitionindex == 0) //in case of logistic growth
	{
		for (int i = 0; i < numlineages; i++)
			growthrates[i] = updaterate * (p[i].growthrate * (1 - (c1 - p[i].biomass * p[i].competitiveness + a_intra * p[i].biomass) / (K * averagec))) * p[i].biomass;
	}
	else //in case of nutrient competition
	{
		double c2, c3; //c2 = competition each lineage faces; c3 = nutrients available to this lineage (depending on how competitive other lineages are)
		for (int i = 0; i < numlineages; i++)
		{
			c2 = c1 - p[i].biomass * p[i].competitiveness + p[i].biomass;
			c3 = K - c2 / averagec;
			growthrates[i] = updaterate * (p[i].growthrate * c3 / (c3 + growth_constant) - p[i].deathrate) * p[i].biomass;
		}
	}
...
void mutate(vector<plant> &p, int x)
{
	...
	if (trait <= mutationrate_amount / totalmutationrate)
		...
		mutatetrait(a.investment_toxicity, mutationstep, 0.00001, maxvalue_toxicity);
