...
#include <stdlib.h>
#include "random.h"
#include "evolutionofdefense_ecologicalcosts_parameters.h"
#include "evolutionofdefense_ecologicalcosts_indvalues.h"
#pragma hdrstop
...
void getparameters(ifstream &infile)
{
	...
	while (!infile.eof())
	{
		...
		if (line == 6)
			mutationrate_toxicity = strtod(stuff.c_str(), &pEnd);
		if (line == 7)
			mutationstep = strtod(stuff.c_str(), &pEnd);
		if (line == 8)
			initialvalue_amount = strtod(stuff.c_str(), &pEnd);
		if (line == 9)
			initialvalue_toxicity = strtod(stuff.c_str(), &pEnd);
		if (line == 10)
			cost_direct_amount = strtod(stuff.c_str(), &pEnd);
		if (line == 11)
			cost_direct_toxicity = strtod(stuff.c_str(), &pEnd);
		if (line == 12)
			cost_ecological_amount = strtod(stuff.c_str(), &pEnd);
		if (line == 13)
			cost_ecological_toxicity = strtod(stuff.c_str(), &pEnd);
		if (line == 14)
...
int main(int argc, char* argv[])
{
	...
	totalmutationrate = mutationrate_amount + mutationrate_toxicity;
	...
	vector< vector<double> > outputdistribution (3 + numrounds * 2, vector<double> (5000, 0));
