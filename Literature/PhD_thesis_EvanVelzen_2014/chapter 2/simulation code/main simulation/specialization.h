...
void calculatespecialization(animal &herbivore)
{
	...
	for (int i = 0; i < number_plantspecies; i++)
		sp += pow(herbivore.preference[i] / herbivore.totalpreference - 1 / double(number_plantspecies), 2) * number_plantspecies / (number_plantspecies - 1);
...
void initializeanimals(vector<animal> &herbivores)
{
	...
	for (int i = 0; i < number_herbivores; i++)
	{
		for (int j = 0; j < number_plantspecies; j++)
		{
			...
			if (herbivores[i].preference[j] > 1)
				herbivores[i].preference[j] = 1;
...
//---------------------------------------------------------------------------
double plantgrowth(double plantbiomass, double &free_nutrients)
{
...
//---------------------------------------------------------------------------
double plantgrowth_ratio(double plantbiomass, double &free_nutrients, double competition)
{
...
//---------------------------------------------------------------------------
double calculatecompetition(vector<double> &plants, int x) 
{
	...
	{
		if (i == x)
			comp += a1 * plants[i]; 	//intraspecific competition
		else comp += a2 * plants[i];	//interspecific competition
	}
	return comp;
}
//---------------------------------------------------------------------------
double eating(double &plantbiomass, int plant, animal &herbivore, double totalhandlingtime, double &free_nutrients)
{
	//calculate amount consumed
	double amounteaten = updaterate * herbivore.biomass * attackrate * pow(herbivore.preference[plant] / herbivore.totalpreference, n)
		* plantbiomass / (1 + totalhandlingtime);
	...
	if (amounteaten <= plantbiomass)
	{
		...
		free_nutrients += amounteaten * conversionfactor * (1 - conversion_efficiency) * quality;
		return amounteaten;
	}
...
//---------------------------------------------------------------------------
void ecologicaldynamics(vector<double> &plants, vector<animal> &herbivores, vector<int> &cons, double &free_nutrients)
{
	...
	for (int i = 0; i < plants.size(); i++)
	{
		if (version == 0) //in case of "normal" not-competition growth
			growthrates[i] = plantgrowth(plants[i], free_nutrients);
		else growthrates[i] = plantgrowth_ratio(plants[i], free_nutrients, calculatecompetition(plants, i)); //in case of ratio-dependent growth
	}
	for (int i = 0; i < plants.size(); i++)
	{
		plants[i] += growthrates[i];
	...
	for (int i = 0; i < herbivores.size(); i++)
	{
		...
		for (int j = 0; j < plants.size(); j++)
			...
			* pow(herbivores[cons[i]].preference[j] / herbivores[cons[i]].totalpreference, n) * plants[j];
		for (int j = 0; j < plants.size(); j++)
			plants[j] -= eating(plants[j], j, herbivores[cons[i]], totalhandlingtime, free_nutrients);
...
void mutate(animal &herbnew)
{
	...
	for (int i = 0; i < number_plantspecies; i++)	//some mutation is introduced for all preference traits
	{
		...
		if (herbnew.preference[i] > 1)
			herbnew.preference[i] = 1;
	}
...
//---------------------------------------------------------------------------
void generateabundanceseries(vector< vector<double> > &output, vector<double> &plants, vector<animal> &herbivores, double &free_nutrients, int time, int replica)
{
	...
	//store abundances of plants and herbivores and total free nutrients to write to file later 
	if (replica == 0)
		output[0][time - 1] = time * 10;
	output[(number_plantspecies + 2) * replica + number_plantspecies + 1][time - 1] = totalherbivorebiomass;
	output[(number_plantspecies + 2) * replica + number_plantspecies + 2][time - 1] = free_nutrients;
	for (int i = 0; i < number_plantspecies; i++)
		output[(number_plantspecies + 2) * replica + i + 1][time - 1] = plants[i];
}
//---------------------------------------------------------------------------
...
void generateindividualvalues(vector< vector<double> >&output, vector<animal> &herbivores, int replica)
{
	...
	//save individual values for species, preference, specialization, and biomass to write to file later
	for (int i = 0; i < number_herbivores; i++)
	{
...
//---------------------------------------------------------------------------
void output_timeseriessp (vector<double> &plants, vector<animal> &herbivores, vector< vector<double> > &output, int generation, int outputrate, int replica)
{
...
//---------------------------------------------------------------------------
void output_abundanceseries (ofstream &outfile1, vector< vector<double> > &output) //write abundance timeseries to textfile
{
	//write headers to file
	outfile1 << "time\t";
	...
	{
		for (int j = 0; j < (number_plantspecies + 2) * number_replicas + 1; j++)
			outfile1 << output[j][i] << "\t";
		outfile1 << endl;
	}
}
