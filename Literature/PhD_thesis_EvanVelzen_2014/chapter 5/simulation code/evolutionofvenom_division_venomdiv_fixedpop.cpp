...
struct parasitoid
{
	...
	double fitness;		//relative fitness: number of offspring divided by highest number of offspring in parasitoid population
};
...
int hostpopulationsize;		//number of hosts
int parasitoidpopulationsize;		//number of parasitoids
...
void getparameters(ifstream &infile)
{
	...
	while (!infile.eof())
	{
		...
		if (line == 6)
			hostpopulationsize = strtol(stuff.c_str(), &pEnd, 10);
		if (line == 7)
			parasitoidpopulationsize = strtol(stuff.c_str(), &pEnd, 10);
		if (line == 8)
...
{
	//host reproduction
	//population size remains fixed; just reset all hosts to initial conditions
...
//---------------------------------------------------------------------------
int findmaxoffspring(vector<parasitoid> &p)
{
	//find and return the number of offspring of the individual with the highest fitness
	int max = 0;
	for (int i = 0; i < p.size(); i++)
	{
		if (p[i].numoffspring > max)
			max = p[i].numoffspring;	
	}
	return max;
}
//---------------------------------------------------------------------------
...
{
	//parasitoid reproduction: evolution
	//generate the new generation
	//first, calculate fitness of all parasitoids (number of offspring relative to parasitoid with most offspring, which has a fitness of 1)
	
	int maxfitness = findmaxoffspring(p);
	for (int i = 0; i < p.size(); i++)
		p[i].fitness = double(p[i].numoffspring) / double(maxfitness);
	
	//new generation; parent parasitoid "chosen" for each offspring of the new generation
	vector<parasitoid> nextgen (parasitoidpopulationsize);
	int chosenparasitoid;
	
	//for each individual in the offspring generation, a parent is chosen with a probability determined by its fitness	
	for (int i = 0; i < nextgen.size(); i++)
	{
		do
			chosenparasitoid = RandomNumber(parasitoidpopulationsize);
		while (p[chosenparasitoid].fitness < Uniform()); //chosen parasitoid is rejected as parent with a probability that increases as its fitness decreases; in that case, a new parent is chosen
		nextgen[i] = p[chosenparasitoid]; //if parent parasitoid is accepted, offspring inherits its trait values
		if (mutationrate > Uniform())	  //with some mutation, with a small probability
			mutate(nextgen[i]);
		updateparasitoid(nextgen[i]);     //clutch sizes etc. are calculated based on new trait values
	}
	p = nextgen; //offspring population becomes new current generation
}
//---------------------------------------------------------------------------
...
//--------------------------------------------------------------------------
void maketimeseries(vector< vector<double> > &output, vector<parasitoid> &p, int run, int generation)
{
	//make time series, storing average trait values every 100 generations
	if (run == 0)
		output[0][generation - 1] = generation * 100;
	output[run * 3 + 1][generation - 1] = calculateaveragev(p);
	output[run * 3 + 2][generation - 1] = calculateaveragevdiv(p);
	output[run * 3 + 3][generation - 1] = calculateaveraged(p);
}
//---------------------------------------------------------------------------
void makeoutput_end(vector< vector<double> > &output, vector<parasitoid> &p, int run)
{
	//store average trait values at the end of simulation
	output[run][0] = calculateaveragev(p);
	output[run][1] = calculateaveragevdiv(p);
	output[run][2] = calculateaveraged(p);
}
//---------------------------------------------------------------------------
...
int main(int argc, char* argv[])
{
	...
	//2D-arrays in which output is stored: time series, distribution of traits in population, averages at the end of each simulation run
	vector< vector<double> > output_timeseries(numruns * 3 + 1, vector<double> (numgenerations / 100, 0));
	vector< vector<double> > output_distribution(numruns * 3 + 2, vector<double> (5000, 0));
	vector< vector<double> > output_end(numruns, vector<double> (3, 0));

	//create hosts and parasitoids
	vector<parasitoid> parasitoids (parasitoidpopulationsize);
	vector<host> hosts (hostpopulationsize);
	calculatefirst(parasitoidpopulationsize);	//calculate fraction of encounters that involves unparasitized hosts
	...
	for (int run = 0; run < numruns; run++)
	{
		...
		for (int i = 0; i < numgenerations; i++)
		{
			if ((i+1) % 100 == 0) //make time series every 100 generations
				maketimeseries(output_timeseries, parasitoids, run, (i+1) / 100);
		...
		makeoutput_end(output_end, parasitoids, run); //store average trait values at end of simulation
	}
	...
	for (int i = 0; i < numruns; i++)
		outfile1 << "av v\tav div v\tav div\t"; //write headers to file
	outfile1 << endl;
