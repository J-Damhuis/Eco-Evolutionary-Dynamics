...
void updateparasitoid (parasitoid &x)
{
	...
	// clutch size unparasitized and parasitized
	...
	for (int i = 0; i < h.size(); i++) //for each host
	{
		for (int j = 0; j < parasitizations[i].size(); j++) //go over each encounter one by one
		{
			if (h[i].parasitized == false) //if the host is unparasitized
			{
				...
				{
					h[i].totalvenom += p[parasitizations[i][j]].venomousness; 	//inject venom
...
void mutate(parasitoid &x)
{
	...
	switch(RandomNumber(2)) //one of the traits is randomly chosen; then new value for that trait is drawn from normal distribution with s.d. = mutationstep
	{
		case 0: x.venomousness += Normal (0, mutationstep); break;
		case 1: x.division_f += Normal (0, mutationstep); break;
	}
	
	//keep venom production above 0, and division between 0 and 1
	if (x.venomousness < 0)
		x.venomousness = 0;
	if (x.division_f < 0)
		x.division_f = 0;
...
void plotdistribution(vector<parasitoid> &p, vector< vector<double> > &output, int generation, int run, int time)
{
	...
	for (int i = 0; i < 50; i++)
	{
		...
		output[run * 2 + 2][time * 50 + i] = traitcategories1[i];
		output[run * 2 + 3][time * 50 + i] = traitcategories2[i];
	}              
}
//--------------------------------------------------------------------------
void maketimeseries(vector< vector<double> > &output, vector<host> &h, vector<parasitoid> &p, int run, int generation)
{
	...
	output[run * 4 + 1][generation - 1] = h.size();
	output[run * 4 + 2][generation - 1] = p.size();
	output[run * 4 + 3][generation - 1] = calculateaveragev(p);
	output[run * 4 + 4][generation - 1] = calculateaveraged(p);
}
//---------------------------------------------------------------------------
void makeoutput_end(vector< vector<double> > &output, vector<host> &h, vector<parasitoid> &p, int run)
{
	...
	output[run][3] = calculateaveraged(p);
}
//---------------------------------------------------------------------------
...
int main(int argc, char* argv[])
{
	...
	vector< vector<double> > output_timeseries(numruns * 4 + 1, vector<double> (numgenerations / 100, 0));
	vector< vector<double> > output_distribution(numruns * 2 + 2, vector<double> (5000, 0));
	vector< vector<double> > output_end(numruns, vector<double> (4, 0));
	...
	for (int i = 0; i < numruns; i++)
		outfile1 << "hosts\tparasitoids\tav v\tav div\t"; //write headers to file
	outfile1 << endl;
	...
	outfile2 << "time\tvalue\t";
	for (int i = 0; i < numruns; i++)
		outfile2 << i + 1 << " v\tdiv\t"; //write headers to file
	outfile2 << endl;
	for (int i = 0; i < 5000; i++)
	{
		for (int j = 0; j < numruns * 2 + 2; j++)
